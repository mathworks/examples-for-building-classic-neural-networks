...
%
%  View the derivative of a simple two-dimensional surface from any
%  point and any direction.

% Copyright 1994-2014 Martin T. Hagan
...
if isempty(fig)
  ...
  H = nndArray({fig_axis desc_text big little marker_ptr position_ptr line_ptr derivative_ptr});
  set(fig,'userdata',H)
...
if strcmp(cmd,'down')
  ...
  delete(marker);
  marker = [plot(x1,x2,'ok','markersize',marker_size,'CreateFcn','');
            plot(x1,x2,'or','markersize',marker_size+2,'CreateFcn','');
            plot(x1,x2,'ob','markersize',marker_size+4,'CreateFcn','');
    plot(x1,x2,'ok','markersize',marker_size+6,'CreateFcn','')];
...
elseif strcmp(cmd,'motion')
  ...
  if ishghandle(the_line), delete(the_line); end % MHB
  axes(big);
  set(big,'nextplot','add'); % MHB
  set(gcf,'nextplot','add'); % MHB
  the_line = plot3([x1 y1],[x2 y2],[1 1],'color','b','CreateFcn','','linewidth',2);
  set(line_ptr,'userdata',the_line);
  set(gcf,'nextplot','new'); % MHB
  ...
  if ishghandle(der) % MHB
    set(der,'facecolor',nnltyell);
    delete(der);
  end
  axes(little)
  set(little,'nextplot','add'); % MHB
  set(gcf,'nextplot','add'); % MHB
  der = fill3([1 1 0],[-1 1 0]+dir_der,[1 1 1],nndkblue,...
    'edgecolor','none',...
    'CreateFcn','');
  %der = patch([0 1 1 0],[0 0 dir_der dir_der],'b','CreateFcn','');
  set(little,'nextplot','add'); % MHB
  set(gcf,'nextplot','new'); % MHB
