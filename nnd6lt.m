...
%
%  Define two vectors and their transforms, and see the resulting
%  eigenvectors of the general linear transform.
...
if ~isempty(fig)
  ...
  trans_ptr = H(5);           % handles to transformation vectors
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  H = nndArray({fig_axis desc_text left right trans_ptr click_ptr ...
...
elseif strcmp(cmd,'trans') & ~isempty(fig)
...
elseif strcmp(cmd,'down') & ~isempty(fig)
...
elseif strcmp(cmd,'leftdown') & ~isempty(fig) & (nargin == 3)
  ...
  if (number < 2)
    ...
    track_h = plot([x x],[y y],'CreateFcn','','linewidth',2);
...
elseif strcmp(cmd,'leftmotion') & ~isempty(fig)
  axes(left)
  [in,x,y] = nnaxclik(left);
...
elseif strcmp(cmd,'leftup') & ~isempty(fig)
  axes(left)
  ...
  % REMOVE TRACK LINE
  set(track_data(3),'CreateFcn','','color',ltyell);
  delete(track_data(3));
...
elseif strcmp(cmd,'rightdown') & ~isempty(fig) & (nargin == 3)
  axes(right)
...
elseif strcmp(cmd,'rightmotion') & ~isempty(fig)
  axes(right)
  set(fig,'nextplot','add')
...
elseif strcmp(cmd,'rightup') & ~isempty(fig)
  axes(right)
  set(fig,...
...
elseif strcmp(cmd,'clear') & ~isempty(fig)
