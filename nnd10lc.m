...
%
%   Define six input images associated with three target values, train
%   a linear network, then test it on a seventh image.
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  H = nndArray({fig_axis desc_text pattern P_ptr w_ptr select indicator center ...
...
elseif strcmp(cmd,'select') & ~isempty(fig) & (nargin == 2)
...
elseif strcmp(cmd,'weight') & ~isempty(fig)
...
if strcmp(cmd,'train') & ~isempty(fig)
...
if strcmp(cmd,'meter') & ~isempty(fig)
