...
%
%  Define vectors, and view their transformed version, until you
%  either run out of turns or find the transforms two eigenvectors.
...
if isempty(fig)
  ...
  for i=0:10
    ...
    smiles(i+1) = plot(xx,yy,...
      'color',nnyellow,'CreateFcn','','linewidth',4,'visible','off');
  end
  ...
  set(little,...
    ...
    'ylim',[0 10.2],...
    'ytick',[0 2 4 6 8 10])
  bar = fill([0 1 1 0]*0.6+0.2,[0 0 1 1]*10,nnred,'CreateFcn','');
  ...
  H = nndArray({fig_axis desc_text big little bar smiles_ptr angle_ptr ...
  ...
  nnsettxt(desc_text,...
    ...
    'chosen will appear in red.  The result of transforming this vector will be black.',...
    'Release the button and try to click so the red and black vectors point in',...
...
elseif strcmp(cmd,'up')
  ...
  % FIRST ANSWER FOUND
  if (length(angles) == 1) && isempty(first)
    if (angles == answer(1))
      angle = answer(2);
    else
      angle = answer(1);
