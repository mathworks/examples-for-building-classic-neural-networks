...
%
%  Watch the dynamic response of a pendulum started from various initial
%  conditions and energies as it settles down.
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  H = nndArray({fig_axis desc_text cont_axis pend_axis pend ball cont_ptr...
...
elseif strcmp(cmd,'drag_pend') & ~isempty(fig)
...
elseif strcmp(cmd,'drop_pend') & ~isempty(fig)
...
elseif strcmp(cmd,'move_start') & ~isempty(fig)
...
elseif strcmp(cmd,'go') & ~isempty(fig)
  ...
  [T,Y] = ode45('nndpend',[0:0.05:Tfinal], Y0,options);
  angles = Y(:,1)';
  velocities = Y(:,2)';
