...
%
%  Select a point on a surface and see the gradient approximation of the
%  surface around the initial point with a first step taken down the
%  gradient.  Also see the path taken by iteratively following the
%  steepest gradient.
...
if isempty(fig)
  ...
  H = nndArray({fig_axis desc_text left right marker_ptr point_ptr path_ptr ...
    lr_slider lr_text});
  set(fig,'userdata',H)
